<?php
	...
	$limit = 2;
	$offset = ($page-1)*$limit;
    $sql = "SELECT * FROM blog LEFT JOIN categories ON blog.category=categories.cat_id LEFT JOIN user ON blog.author_id=user.user_id WHERE blog.blog_title LIKE '%$keyword%' OR blog.blog_body LIKE '%$keyword%' ORDER BY blog.publish_date DESC LIMIT $offset, $limit";
    $query = mysqli_query($config, $sql);
    $rows = mysqli_num_rows($query);
?>
<div class="container mt-2">
	<div class="row">
        <h4>Search result for: <span class="text-primary"><?= $keyword ?></span></h4>
		<div class="col-lg-8">
            <hr>
        <?php
        ...
        ?>
            ...
            <?php } }else{echo "
                    <h5 class='text-danger display-4'>No Record Found</h5>
                    <b>Suggestions:</b>
                    <ul>
			...
			<?php
				...
				$pagination = "SELECT * FROM blog WHERE blog.blog_title LIKE '%$keyword%' OR blog.blog_body LIKE '%$keyword%'";
				...
				<ul class="pagination mt-3">
					...
					<?php for ($i=1; $i <= $pages; $i++) { ?>
						<li class="page-item <?= ($i == $page) ? $active = "active" : "" ?>">
							<a class="page-link" href="search.php?keybord=<?= $keyword ?>&page=<?= $i ?>"><?= $i ?></a>
						</li>
					<?php }  ?>
					<!-- <li class="page-item">
